...
inline m4 PosMat(f32 X, f32 Y, f32 Z)
{    
    m4 Result = IdentityM4();
    Result.v[3].x = X;
    Result.v[3].y = Y;
...
inline m4 SetCamera(v3 Target, v3 Up, v3 Pos)
{
...
inline m4 PerspProjMat(f32 AspectRatio, f32 Fov, f32 Near, f32 Far)
{
    m4 Result = {};
...
inline m4 OrthoProjMat(f32 Left, f32 Right, f32 Top, f32 Bottom, f32 Near, f32 Far)
{
    m4 Result = {};
    Result.v[0].x = 2.0f / (Right - Left);
    Result.v[1].y = 2.0f / (Top - Bottom);
    Result.v[2].z = 2.0f / (Far - Near);
    Result.v[3].x = -(Right + Left) / (Right - Left);
    Result.v[3].y = -(Top + Bottom) / (Top - Bottom);
    Result.v[3].z = -(Far + Near) / (Far - Near);
    Result.v[3].w = 1.0f;
