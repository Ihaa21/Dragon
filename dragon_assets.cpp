        ...
        // NOTE: Copy glyph array
        {
            ...
            for (u32 GlyphIndex = 0; GlyphIndex < NumGlyphsToLoad; ++GlyphIndex)
            {
                ...
                CreateImage(&GlobalState.GpuAllocator, FileGlyph.Width, FileGlyph.Height, VK_FORMAT_R8G8B8A8_UNORM,
                            ...
                            &AssetGlyph.View);
                MoveImageToGpuMemory(AssetGlyph.Handle, sizeof(u32), FileGlyph.Width, FileGlyph.Height, Mem + FileGlyph.PixelOffset);
    ...
    for (u64 ModelIndex = 0; ModelIndex < Assets.ModelCount; ++ModelIndex)
    {
        ...
        for (u32 TextureId = 0; TextureId < FileModel.NumTextures; ++TextureId)
        {
            ...
            CreateImage(&GlobalState.GpuAllocator, FileTexture->Width, FileTexture->Height, VK_FORMAT_R8G8B8A8_UNORM,
                        ...
                        &Texture->View);
            MoveImageToGpuMemory(Texture->Handle, sizeof(u32), FileTexture->Width, FileTexture->Height, Mem + FileTexture->PixelOffset);
        }
        ...
        for (u32 MeshId = 0; MeshId < Model->NumMeshes; ++MeshId)
        {
            ...
            if (FileMesh->NumVertices != 0)
            {
                ...
                MoveBufferToGpuMemory(Mesh->Handle, VertexBufferSize, Mem + FileMesh->VertexArrayOffset);
            }
